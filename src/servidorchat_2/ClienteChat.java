...
public class ClienteChat extends JFrame {
    ...
    private String usuario_intro = "z-";
    ...
    public ClienteChat() {
        ...
        if (!usuarios.contains(usuario)) {
            ...
            btEnviar.addActionListener(new ConexionServidor(socket, tfMensaje, usuario));//Aki mando el usuario a conexionServidor
        }
        btGrupo.addActionListener(new ActionListener() {

            //Creo las ventanas del grupo
            @Override
            public void actionPerformed(ActionEvent e) {
                ...
                try {
                    ...
                    for (int i = 0; i < num; i++) {
                        name = (jop.showInputDialog("Ingresa el usuario Nº" + (i + 1)));
                        nombres_admitidos.add(name + ": ");
                    }

                    if (nombres_admitidos.size() > 0) {
                        for (int i = 0; i < nombres_admitidos.size(); i++) {
                            usuario_intro += nombres_admitidos.get(i) + "-";
                        }
                    }
                    salidaDatos = new DataOutputStream(socket.getOutputStream());
                    salidaDatos.writeUTF(usuario_intro);///Aqui se lleva el String con los nombres
